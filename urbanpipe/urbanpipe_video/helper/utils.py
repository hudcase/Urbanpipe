...
def train(model, trainloader, criterion, optimizer, epoch, device):
    ...
    返回:
        tuple: 包含训练损失、训练准确率、训练mAP和预测标签的元组。
    """
    model.train()
    running_loss = 0.0
    y_true_train = []
    ...
    for data in tqdm(trainloader):
        ...
        y_pred_train.extend(torch.sigmoid(outputs).cpu().detach().numpy().round())
...
def test(model, testloader, criterion, device):
    ...
    with torch.no_grad():
        for data in tqdm(testloader):
            ...
            y_pred_test.extend(torch.sigmoid(outputs).cpu().numpy().round())
